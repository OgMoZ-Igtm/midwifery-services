...
def render_cultural_carousel():
    """
    CORRIGÉ (POINT 4): Affiche le carrousel culturel en utilisant HTML/JS
    pour gérer le défilement automatique sans bloquer le thread Streamlit.
    """
    st.subheader("📚 Fiches culturelles")

    # Génération du code HTML/JS pour le carrousel
    html_code = generate_carousel_html(fiches)

    # Affichage du carrousel via Streamlit (nécessite l'implémentation de generate_carousel_html)
    st.html(html_code)
...
def render_home_page():
    ...
    st.markdown("---")
    st.subheader("🔊 Le souffle de la vie")
    play_audio_if_enabled("bebe_pleure.mp3")
...
def generate_carousel_html(fiches):
    """
    Génère le HTML du carrousel culturel avec Swiper.js.
    CORRIGÉ (POINT 4) : Défilement unique (slidesPerView: 1) de droite à gauche (reverseDirection: true) toutes les 6 secondes (delay: 6000).
    """
    slides = ""
    for fiche in fiches.values():
        # Utilisation de la fonction image_to_base64 pour assurer l'affichage des images
        ...
        if img_data:
            ...
            <div class="swiper-slide" style="display:flex; flex-direction:column; justify-content:center;">
                <h3>{fiche['titre']}</h3>
                <div style="height:260px; background-color:#F0F0F0; border-radius:10px; display:flex; align-items:center; justify-content:center; width: 45%; margin: auto;">
    ...
    # Le code HTML/JS utilise Swiper pour un défilement automatique toutes les 6 secondes
    html_code = f"""
    ...
    <style>
        ...
        /* Ajustement des flèches de navigation pour la direction RTL */
    ...
    <script>
        const swiper = new Swiper('.swiper', {{
            loop: true,
            autoplay: {{
                delay: 6000, /* CORRIGÉ (POINT 4): Délai de 6000ms (6 secondes) */
                disableOnInteraction: false,
            }},
            slidesPerView: 1, /* CORRIGÉ (POINT 4): Affichage d'image unique */
            spaceBetween: 30,
            speed: 1000,
            reverseDirection: true, /* CORRIGÉ (POINT 4): Défilement de droite à gauche */
            navigation: {{
...
def render_sidebar_auth(role: str):
    """Affiche la barre latérale pour un utilisateur authentifié."""
    ...
    # Initialisation avant le widget
    if "main_section_auth" not in st.session_state:
        st.session_state["main_section_auth"] = "Accueil"  # Par défaut

    # Widget radio : on ne réassigne pas dans session_state
    main_section_auth = st.sidebar.radio(
        "Navigation principale",
        sections,
        key="main_section_auth",  # Clé explicite
    ...
    if st.sidebar.button("Déconnexion 🚪"):
        ...
        # Supprimer les clés spécifiques à l'utilisateur
        st.session_state.pop("active_form", None)
        st.session_state.pop("active_form_title", None)
        st.session_state.pop("active_shared_func", None)
        st.session_state.pop("active_shared_func_title", None)
        st.session_state.pop(
            "main_section_auth", None
        )  # Important pour éviter le bug lors de la prochaine connexion
        st.rerun()
...
def main():
    ...
    # 2. Gestion de l'état (Connecté ou non)
    if st.session_state["authenticated"]:
        # --- État Authentifié ---

        # 2a. Rendu de la barre latérale (Navigation)
        role = st.session_state["role"]
        render_sidebar_auth(role)

        # 2b. Rendu du corps principal (Contenu)
        # S'assure que la sélection par défaut est 'Accueil'
        selected_section = st.session_state.get("main_section_auth", "Accueil")
        render_section(selected_section, role)

    else:
        # --- État Non-Authentifié (Public) ---

        # 2a. Rendu de la barre latérale (Formulaire de connexion)
        render_login_form()

        # 2b. Rendu du corps principal (Contenu Public)
        render_public_header()
        # CORRIGÉ (POINT 3): Le contenu public n'appelle pas le carrousel/météo de la page d'accueil authentifiée.
        render_info_section()
